...
if chosen_parameter_name == 'H2 inlet temperature':
    streams_in[0].T = parameter_value
...
final_stage_props = all_component_inputs['Properties'][all_component_inputs['Stages']]; #print(final_stage_props)
...
h_H2_out = CP.PropsSI('Hmass','T',stage_props[0],'P',stage_props[1],'H2') # [J/kg] mass specific enthalp of H2
...
if all_component_inputs['Type'] == 'diaphragm': # assumes isothermal compressor work
    Wdot = mdot_H2*R_H2*((streams_in[0].T+stage_props[0])/2)*ln(stage_props[1]/streams_in[0].P); print('Isothermal work = ', Wdot, ' [W]') # [W]
elif all_component_inputs['Type'] == 'reciprocating': # assumes adiabatic compressor work
    Wdot = mdot_H2*((k*R_H2*streams_in[0].T)/(k-1))*(((stage_props[1]/streams_in[0].P)**((k-1)/k)) -1); print('Adiabatic work = ', Wdot, ' [W]') # [W]
...
streams_out[0].T = stage_props[0] # [K] head outlet temperature is per inputed specs
streams_out[0].P = stage_props[1] # [Pa] head outlet pressure is per inputed specs
