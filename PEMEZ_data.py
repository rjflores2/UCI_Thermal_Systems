...
array_in_labels = ['stream #', 'i [A/cm^2]', 'model #', 'T_in [K]', 'P_in [Pa]','N_in [mol/s]', 'molar fraction KOH', 'molar fraction H2O_l', 'molar fraction H2O_v', 'molar fraction H2', 'molar fraction O2','molar fraction N2']
array_out_labels = ['stream #','i [A/cm^2]', 'model #', 'T_out [K]', 'P_out [Pa]','N_out [mol/s]', 'molar fraction KOH', 'molar fraction H2O_l', 'molar fraction H2O_v', 'molar fraction H2', 'molar fraction O2','molar fraction N2']
PEMEZ_data = xlsxwriter.Workbook('PEMEZ_data.xlsx')
...
for y in range(len(components_list)):
    ...
    sheet = AEZ_data.add_worksheet(component)
...
if len(current_range) == 1: # checks to see if you are solving for only 1 current density
    ...
    for z in range(len(i_range_pol)):
        i = i_range_pol[z]
        U_cell_array = U_cell(w, P_op_bar, T_in, T_op, i, model); #print(U_cell_array)
...
#---------------------------------------
print('PEMEZ_data.py has finished running.')