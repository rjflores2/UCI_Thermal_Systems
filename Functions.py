...
def rxn_H(P, T_in, T_out):
    """ (float, float) -> float
    Return the molar enthalpy of the electrochemical reaction at specified operating pressure and inlet and outlet temperatures.
    P_in is in [Pa]
    ...
    H2_Cp_in_fun = Cp_mol('H2', P, T_in) # [J/mol/K]
    O2_Cp_in_fun = Cp_mol('O2', P, T_in);  # [J/mol/K] 
    return (1*(H2_HformSTD + ((H2_Cp_in_fun + Cp_mol('H2', P, T_out))/2)*(T_out-T_in)) # H2_Hform (1/3) rxn_H 
         +0.5*(O2_HformSTD + ((O2_Cp_in_fun + Cp_mol('O2', P, T_out))/2)*(T_out-T_in)) # O2_Hform (2/3) rxn_H 
        -(1*H2O_HformSTD)) # (3/3) rxn_H 

#---------------------------------------

def rxn_S(P, T_in, T_out):
    """ (float, float) -> float
    ...
    H2_Cp_in_fun = Cp_mol('H2', P, T_in) # [J/mol/K]
    O2_Cp_in_fun = Cp_mol('O2', P, T_in);  # [J/mol/K] 
    return (1*(H2_SformSTD + ((H2_Cp_in_fun + Cp_mol('H2', P, T_out))/2)*(T_out-T_in)/((T_in+T_out)/2))  # H2_Sform (1/3) rxn_S 
            +0.5*(O2_SformSTD + ((O2_Cp_in_fun + Cp_mol('O2', P, T_out))/2)*(T_out-T_in)/((T_in+T_out)/2)) # O2_Sform (2/3) rxn_S 
...
def Cp_mass(gas, P, T):
   """ (string, float, float) -> number
   Return the mass specific consant pressure specifc heat of the desired gas a function of temperature and pressure.
   Pulled from COOL PROP library.
   T is in [K], Cp is in [J/kg/K]
   """
   import CoolProp.CoolProp as CP; 
   if gas == 'H2':
       Cp_mass_CP = CP.PropsSI('Cpmass','T',T,'P',P,'H2'); 
   elif gas == 'O2':
       Cp_mass_CP = CP.PropsSI('Cpmass','T',T,'P',P,'O2'); 
   elif gas == 'H2O_L':
       Cp_mass_CP = CP.PropsSI('Cpmass','T|liquid',T,'P',P,'H2O'); 
   elif gas == 'H2O_V':
       Cp_mass_CP = CP.PropsSI('Cpmass','T|gas',T,'P',P,'H2O');
   return Cp_mass_CP
...
def Cp_mol(gas, P, T):
   """ (string, float, float) -> number
   Return the molar specific consant pressure specifc heat of the desired gas a function of temperature and pressure.
   Pulled from COOL PROP library.
   T is in [K], Cp is in [J/mol/K]
   """
   import CoolProp.CoolProp as CP; 
   if gas == 'H2':
       Cp_molar_CP = CP.PropsSI('Cpmolar','T',T,'P',P,'H2'); 
   elif gas == 'O2':
       Cp_molar_CP = CP.PropsSI('Cpmolar','T',T,'P',P,'O2'); 
   elif gas == 'H2O_L':
       Cp_molar_CP = CP.PropsSI('Cpmolar','T|liquid',T,'P',P,'H2O'); 
   elif gas == 'H2O_V':
       Cp_molar_CP = CP.PropsSI('Cpmolar','T|gas',T,'P',P,'H2O');
   return Cp_molar_CP

#---------------------------------------

def Cp_mass_ideal_CP(gas, P, T):
   """ (string, float, float) -> number
   Return the ideal mass specific consant pressure specifc heat of the desired gas a function of temperature and pressure.
   Pulled from COOL PROP library.
   T is in [K], Cp is in [J/kg/K]
   """
   import CoolProp.CoolProp as CP; 
   if gas == 'H2':
       Cp_mass_ideal_CP = CP.PropsSI('Cp0mass','T',T,'P',P,'H2'); 
   elif gas == 'O2':
       Cp_mass_ideal_CP = CP.PropsSI('Cp0mass','T',T,'P',P,'O2'); 
   elif gas == 'H2O_L':
       Cp_mass_ideal_CP = CP.PropsSI('Cp0mass','T|liquid',T,'P',P,'H2O'); 
   elif gas == 'H2O_V':
       Cp_mass_ideal_CP = CP.PropsSI('Cp0mass','T|gas',T,'P',P,'H2O');
   return Cp_mass_ideal_CP
...
if __name__ == '__main__':
    ...
    i =0.2
    print('Cp_mass_ideal_text = ', Cp_mass_ideal_text('H2', T), '[J/kg/K]')
    print('Cp_mol_ideal_text = ', Cp_mol_ideal_text('H2', T), '[J/mol/K]')
    print('h_CP = ', h_CP('H2O_L', P, T), '[J/kg]')
    print('rxn_H = ', rxn_H(P,T_in,T_out), '[J/mol]')
    print('rxn_S = ', rxn_S(P,T_in,T_out), '[J/mol/K]')
    print('Cv_mass = ', Cv_mass('H2', P, T), '[J/kg/K]')
    print('Cp_mass = ', Cp_mass('H2', P, T), '[J/kg/K]')
    print('Cp_mol = ', Cp_mol('H2', P, T), '[J/kg/K]')
    print('Cp_mass_ideal = ', Cp_mass_ideal_CP('H2', P, T), '[J/kg/K]')